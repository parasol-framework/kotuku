...
indexterm:[loops, numeric for] `for Name = Start, Stop [, Step] do pass:[...] end`
...
`{Start to Stop}` is an exclusive range: the stop value is not included.  `{Start into Stop}` is an inclusive range: the stop value is included.  The step direction is determined automatically from the start and stop values.
...
indexterm:[return statement] `return` exits the current function and returns zero or more values to the caller.  `return` with no values returns `nil`.
