// VERIFIED: 2026-02-10
// UPDATED: 2026-05-24 — Added regex string shorthand (r'...') to String Literals section.

[[ch-basic-syntax]]
== Basic Syntax
indexterm:[syntax] indexterm:[lexical conventions] This chapter defines the lexical structure of Tiri: how source text is decomposed into tokens, and the rules governing comments, identifiers, keywords, literals, statements, and blocks.

[[sec-comments]]
=== Comments
indexterm:[comments] The lexer discards all comment text.  Comments have no effect on program behaviour.  Tiri supports three comment forms.
...
indexterm:[identifiers] indexterm:[keywords] indexterm:[reserved words] An identifier is a name used to refer to variables, functions, fields, and other user-defined entities.
...
|`end`     |`enum`     |`except`   |`false`    |`for`
|`from`    |`function` |`global`   |`has`      |`if`
|`import`  |`in`       |`is`       |`local`    |`namespace`
|`nil`     |`not`      |`or`       |`raise`    |`repeat`
|`return`  |`success`  |`then`     |`thunk`    |`true`
|`try`     |`until`    |`when`     |`while`    |`with`
|===
...
indexterm:[statements] indexterm:[blocks] A statement forms the basic unit of execution.  Tiri supports assignment statements, function calls, control-flow statements, and declarations.
...
When a numeric literal is immediately followed by `to` or `into`, the lexer recognises a range expression.  This permits range expressions such as `{1 to 10}`.
...
An `r` prefix before a short string or long string creates a compiled regular expression object.  The expression `r'...'` is equivalent to `regex.new('...')`, but the content is treated as a raw string: backslash sequences are not interpreted as escape characters, so regex metacharacters such as `\d`, `\w`, and `\s` do not require doubling.

[source,tiri]
----
digits = r'\d+'          -- equivalent to regex.new([[\d+]])
word   = r"\w+"          -- double quotes also accepted
ws     = r[[\s+]]        -- long-bracket form
level  = r[=[\d{2}]=]    -- levelled long-bracket form
----

All four delimiter forms -- single-quoted, double-quoted, long-bracket, and levelled long-bracket -- are supported.  The resulting value is a regex object, not a string.  Methods can be chained directly on the literal:
...
If the pattern is syntactically invalid, an exception is thrown at runtime during compilation of the regex object.

See <<ch-regular-expressions>> for the full regex API.
