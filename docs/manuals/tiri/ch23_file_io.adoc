...
indexterm:[library, io] The `io` namespace provides built-in functions for file operations and standard I/O management.  No `import` statement is required to use these functions.
...
=== File Handle Methods
indexterm:[file system, handles] File handles returned by `io.open()` and related functions support the following methods, called with the colon syntax (e.g. `file:read()`).  Operating on a closed file handle throws an exception.
...
indexterm:[file system, object] The `io` standard library covers common file operations, but the Kōtuku File object provides the full range of file system capabilities: folder enumeration, file watching, permissions, timestamps, symbolic links, copying, moving, and deletion.
...
[[sec-file-search]]
=== File Search
indexterm:[file system, search] The FileSearch extension simplifies recursive file and folder searching with support for name filters, content searching, and size/date constraints.  Load it with:
...
|`caseSensitive`
|boolean
|Controls how `nameFilter` is applied.  When omitted or `false`, the candidate file name is converted to lowercase before matching.  String filters and regex patterns must therefore be written for lowercase input.  When `true`, the original file name is used.
...
|`minDate`
|DateTime
|Exclude files with a modification time earlier than this `DateTime` structure.

|`maxDate`
|DateTime
|Exclude files with a modification time later than this `DateTime` structure.

|`scanLinks`
|boolean
|Allow symbolically linked files to be opened for content matching.  Symbolically linked directories are not recursed.
...
Symbolic links to directories are not followed to avoid infinite recursion.

[[sec-file-manager]]
=== File Manager
indexterm:[file system, manager] The File Manager extension provides file management operations with integrated UI feedback.  Progress dialogs appear automatically for long-running operations, and the user is prompted before overwriting existing files.
...
Pastes files from the system clipboard to the destination path `Dest`.  Files marked for deletion by the clipboard are moved instead of copied.  The function delegates the file transfer to `io.ui.copy()` and does not return its status code.
...
[[sec-virtual-volumes]]
=== Virtual Volumes
indexterm:[file system, volumes] Kōtuku uses a virtual volume system that maps logical volume names to physical file system locations.  All file paths in Tiri use the format `volume:path/to/file`.  The volume prefix is resolved at runtime to a platform-native path, providing location independence across operating systems.
