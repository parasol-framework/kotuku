...
[[sec-iteration]]
=== Iteration
indexterm:[tables, iteration] Tiri provides four iterator functions for traversing tables in `for` loops.
...
[[sec-table-library]]
=== Table Library
indexterm:[library, table] All table library functions are accessed through the `table` namespace.
...
Negative indices count backwards from the end of the table: `-1` is the last element, `-2` the second-to-last.  The exclusive/inclusive semantics of the range operator are preserved with negative indices.  `..` remains exclusive and `pass:[...]` remains inclusive.

[source,tiri]
----
t = { 10, 20, 30, 40, 50 }

t[{-3...-1}]   -- {30, 40, 50}  (inclusive)
t[{-3..-1}]    -- {30, 40}      (exclusive)
