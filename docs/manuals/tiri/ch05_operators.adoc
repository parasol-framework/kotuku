...
The `is` operator compares by value for numbers, strings, and booleans.  Arrays also compare by value: two arrays are equal if they share the same element type, length, and contents.  For tables, functions, and userdata, `is` compares by identity -- two distinct objects are never equal unless a `pass:[__]eq` metamethod overrides this.  The runtime invokes the `pass:[__]eq` metamethod only when both operands share the same metatable.
