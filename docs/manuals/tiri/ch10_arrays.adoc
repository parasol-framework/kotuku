...
indexterm:[arrays, fundamentals] Each array declares a fixed element type at creation.  Storing a value of an incompatible type raises an error.
...
|Primitive (`byte`, `int`, `int16`, `int64`, `float`, `double`)
...
tostring(array<string> { 'a', 'b' })       -- 'array<string, 2>'
...
Byte arrays (`byte`, `char`) can be used with the `..` concatenation operator.  The array is converted to a string and concatenated with the other value.  The result is always a string.
...
Non-byte arrays (e.g. `int`, `string`, `table`) cannot be concatenated and will raise an error.
...
indexterm:[arrays, searching] indexterm:[arrays, querying] Searches for `Value` and returns its zero-based index, or `nil` if not found.  With a numeric `Start`, the search begins at that index.  With a range, the search is restricted to the indices selected by the range (supports exclusive, inclusive, negative, stepped, and reverse ranges).
...
arr:slice({-2...-1})    -- array<int> {40, 50}  (inclusive)
arr:slice({-2..-1})     -- array<int> {40}      (exclusive)
...
[.api]`str = arr:concat([Separator], [Format], [Start], [End])`

Concatenates array elements into a string.  `Separator` is inserted between each element (default: empty string).  `Format` is an optional C-style format specifier applied to each element (e.g. `'%d'`, `'%.2f'`, `'%s'`).  `Start` and `End` are optional zero-based, inclusive indexes that restrict the operation to a sub-range of the array.
...
Concatenation is only available for primitive array types that consist of string and number elements.  Attempting to concatenate types like table, struct and object will raise an error.

An error is raised if `Start` or `End` is negative or beyond the array bounds.  If `End` is less than `Start`, an empty string is returned.  Empty arrays are converted to an empty string.

TIP: Compared to other methods of concatenation, arrays provide the most efficient means of concatenating long series of data values in Tiri.
...
Sub-range concatenation with `Start` and `End`:

[source,tiri]
----
arr = array<string> { 'a', 'b', 'c', 'd' }
arr:concat(',', nil, 1, 2)   -- 'b,c'

arr = array<int> { 1, 2, 3, 4 }
arr:concat('-', '%02d', 1, 3)   -- '02-03-04'
----
