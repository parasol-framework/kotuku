...
The `#` operator returns the current element count.  The `is` operator performs deep equality comparison: two arrays are equal if they have the same element type, the same length, and identical contents.
...
a is b   -- true  (same type, length, and contents)
#a       -- 3
----
...
Byte arrays (`byte`, `char`) can be used with the `..` concatenation operator.  The array is converted to a string and concatenated with the other operand.  The result is always a string.
