...
indexterm:[patterns, matching] Literal patterns match using `is` semantics (identity comparison with no implicit type coercion).  Supported literal types: numbers (including negative and floating-point), strings, booleans (`true`, `false`), and `nil`.
