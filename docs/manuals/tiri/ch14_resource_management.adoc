...
NOTE: `<close>` handlers run during error unwinding.  `defer` blocks do NOT run during error unwinding.  For error-safe cleanup, use `<close>` variables (<<sec-to-be-closed-variables>>).
...
Because `<close>` is only meaningful on local variables, the parser treats `Name <close> = Expression` as a local declaration even without the `local` keyword — the same local-by-default rule that applies to ordinary assignments (<<sec-local-by-default>>).  The two forms are semantically identical; `local` is optional but may be used for clarity.
