...
Targets may use safe navigation on the left-hand side.  When a safe-nav target encounters `nil` during the target walk, only that target's store is skipped; the rest of the assignment proceeds normally.

[source,tiri]
----
a?.x, b = 10, 20                -- if a is nil, b still becomes 20
----
...
For multi-target assignment, the RHS value list is still evaluated using the normal assignment rules so that later targets receive their mapped values:
...
If `config` is `nil`, the first store is skipped but `retries` still receives `3`.

Assigning a function value through safe navigation is supported because it is an ordinary assignment:
...
The function-statement form is not supported:

[source,tiri]
----
function obj?.method()          -- parse error
end
----
...
indexterm:[constant variables] The `<const>` attribute prevents reassignment after initialisation.  It applies to both `local` and `global` declarations.  The compiler enforces this constraint at parse time.
...
indexterm:[to-be-closed variables] indexterm:[close variables] The `<close>` attribute marks a local variable for automatic cleanup when its scope exits.  The runtime invokes the value's `__close` metamethod on scope exit, whether normal or due to an error.
