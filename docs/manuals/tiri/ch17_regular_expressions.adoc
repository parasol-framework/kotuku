// VERIFIED: 2026-02-13
// UPDATED: 2026-05-24 — Added regex string shorthand section documenting r'...' syntax.

[[ch-regular-expressions]]
== Regular Expressions
indexterm:[regular expressions] indexterm:[regex] Tiri provides a compiled regular expression interface through the `regex` module.  Pattern objects are compiled once from an ECMAScript-compatible pattern string and reused for matching, searching, replacing, and splitting operations.  The underlying engine supports full Unicode via UTF-8, named and numbered capture groups, lookahead and lookbehind assertions, and inline flag modifiers.
...
indexterm:[regex, creation] Compiles `Pattern` into a reusable regex object.  `Flags` is an optional bitmask of compilation flags (see <<sec-flags-and-options>>).  If `Pattern` is syntactically invalid, an exception is thrown with error code `ERR_Syntax` and a descriptive message.
...
[discrete]
==== Regex String Shorthand

indexterm:[regex, shorthand] The `r` prefix creates a regex object directly from a string literal, combining declaration and compilation into a single expression.  The content is treated as a raw string: backslash sequences are not interpreted as escape characters.

[.api]`rx = r'Pattern'`

The expression `r'...'` is equivalent to `regex.new('...')` with the escaping convenience of a long string.  Single-quoted, double-quoted, long-bracket, and levelled long-bracket delimiters are all supported:

[source,tiri]
----
digits = r'\d+'          -- single-quoted raw string
word   = r"\w+"          -- double-quoted raw string
ws     = r[[\s+]]        -- long-bracket raw string
level  = r[=[\d{2}]=]    -- levelled long-bracket raw string
----

The result is a compiled regex object, not a string.  All regex methods can be called on it directly, including suffix chaining:

[source,tiri]
----
r'\d+'.test('abc123')    -- true
name, id = r'(\w+)-(\d+)'.extract('item-42')
----

If the pattern is syntactically invalid, an exception is thrown at runtime during the implicit `regex.new()` call.  Wrap in a `try` block when the pattern may be incorrect (see <<ch-error-handling>>).

The `r` prefix does not conflict with variables named `r`.  The lexer recognises the regex shorthand only when `r` is immediately followed by a quote character (`'`, `"`) or an opening long-bracket (`[`).  An `r` followed by whitespace, an index operator, or any other token is parsed as an identifier:

[source,tiri]
----
r = { 'zero' }
print(r[0])              -- 'zero' (table index, not regex)

r = function(V) return V end
print(r 'text')          -- 'text' (function call, not regex)
----
...
indexterm:[regex, matching] indexterm:[regex, searching] Returns `true` if the pattern matches anywhere in `Text`, otherwise `false`.  This is the fastest method for a simple presence check; it does not extract match positions or captures.
...
indexterm:[regex, replacement] indexterm:[regex, splitting] Replaces all matches in `Text` with the `Replacement` string.  By default, all matches are replaced; pass `regex.REPLACE_FIRST_ONLY` in `Flags` to replace only the first occurrence.  If no matches are found, `Text` is returned unchanged.
...
indexterm:[regex, flags] Compilation flags modify how the pattern is interpreted and are specified as the second argument to `regex.new()`.  Multiple flags are combined with the bitwise OR operator.
...
=== Regex Properties
indexterm:[regex, properties] Compiled regex objects expose the following read-only properties:
...
indexterm:[regex, performance] Pattern compilation involves parsing and optimisation.  Compiling the same pattern repeatedly in a loop is redundant.  Compile the pattern once and store the object in a variable:
