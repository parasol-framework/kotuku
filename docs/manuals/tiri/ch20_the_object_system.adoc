...
indexterm:[objects, creation] Creates a new instance of the class identified by `ClassName`.  `ClassName` is a case-insensitive string or a numeric class ID.
...
indexterm:[objects, relationships] Kōtuku enforces a strict parent-child hierarchy.  Every object has exactly one parent and may have any number of children.  By default, `obj.new()` creates objects owned by the script.
