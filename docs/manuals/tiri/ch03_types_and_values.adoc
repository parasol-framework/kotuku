...
Arrays are reference types.  The `is` operator compares arrays by content: two arrays are equal if they have the same element type, length, and contents.  Elements are stored in sequential order from index 0; iteration order matches storage order.
