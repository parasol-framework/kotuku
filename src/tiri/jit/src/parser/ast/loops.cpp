...
//
// Uses a lookahead scan to confirm that `to` or `into`, and optional `by`, exist at brace/paren/bracket depth 0
// inside the braces. If confirmed, parses start, stop and step as full expressions and returns a RangeExpr node.
...
ParserResult<ExprNodePtr> AstBuilder::parse_range_in_braces()
{
   ...
   if (not scan_range_literal(this->ctx, scan)) {
