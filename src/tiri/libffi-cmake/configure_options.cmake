...
check_c_source_runs(
   ...
   int main(void) {
      ...
      if (devzero == -1) return 1;
      return mmap(NULL, 4096, PROT_READ|PROT_WRITE, MAP_SHARED, devzero, 0) == (void *)-1 ? 1 : 0;
   }" HAVE_MMAP_DEV_ZERO)
...
check_symbol_exists(memfd_create sys/memfd.h HAVE_MEMFD_CREATE)
...
if (NOT MSVC)
   ...
   set (UNWIND_SECTION_CMD
      ...
      "-c -o conftest.o - 2>&1 && "
      "echo 'extern void foo();int main(){foo();}' | "
      "${CMAKE_C_COMPILER} ${CMAKE_C_FLAGS} conftest.o -xc - 2>&1")

   execute_process(
      COMMAND sh -c "${UNWIND_SECTION_CMD}"
      WORKING_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR}
      OUTPUT_VARIABLE IGNORE
      ERROR_VARIABLE IGNORE
   ...
   execute_process(
      COMMAND sh -c "${HIDDEN_VISIBILITY_CMD}"
      WORKING_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR}
      OUTPUT_VARIABLE IGNORE
      ERROR_VARIABLE IGNORE
...
configure_file(include/fficonfig.cmake.in ${CMAKE_CURRENT_BINARY_DIR}/include/fficonfig.h)
...
foreach (ASM_PATH IN LISTS WIN_ASSEMBLY_LIST)
   ...
   add_custom_command(
      COMMAND "${CMAKE_C_COMPILER}" /nologo /P /EP /I. /I"${CMAKE_CURRENT_SOURCE_DIR}/${ASM_DIRNAME}"
              /Fi"${CMAKE_CURRENT_BINARY_DIR}/${ASM_FILENAME}.asm" /Iinclude /I"${CMAKE_CURRENT_SOURCE_DIR}/include"
              "${CMAKE_CURRENT_SOURCE_DIR}/${ASM_PATH}"
      DEPENDS ${CMAKE_CURRENT_SOURCE_DIR}/${ASM_PATH}
      OUTPUT ${CMAKE_CURRENT_BINARY_DIR}/${ASM_FILENAME}.asm
      COMMENT "Preprocessing ${CMAKE_CURRENT_SOURCE_DIR}/${ASM_PATH}")
   ...
   if (TARGET_PLATFORM MATCHES X86.*)
      ...
      add_custom_command(
         COMMAND "${CMAKE_ASM_MASM_COMPILER}" /Fo "${CMAKE_CURRENT_BINARY_DIR}/${ASM_FILENAME}.obj"
