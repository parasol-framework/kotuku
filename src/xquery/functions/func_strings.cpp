...
{
   if ((Args.size() < 2) or (Args.size() > 3)) return XPathVal(kt::vector<XTag *>());
   ...
   if (load_regex() != ERR::Okay) return XPathVal(kt::vector<XTag *>());
